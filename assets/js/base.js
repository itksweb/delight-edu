...
const checkUncheckSingle = (container, allToggleSelector, cls) => {
  const allToggle = container?.querySelector(allToggleSelector);
  if (allToggle) {
    container.addEventListener("change", (e) => {
      if (e.target.classList.contains(cls)) {
        const all = container.querySelectorAll(`.${cls}`); //get all checkboxes in boxContainer
...
/*=========================================
        ==>      BULK ACTION  
==========================================*/
const bulkAction = (applyBulkBtn, actionSelector, selectedSelector) => {
  applyBulkBtn?.addEventListener("click", () => {
    // 1. Get the action value
    const action = document.querySelector(actionSelector)?.value;

    // 2. Get selected IDs (replaces .map().get())
    const selectedCheckboxes = document.querySelectorAll(selectedSelector);
